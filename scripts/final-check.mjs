...
check('Versione package 1.6.9', pkg.version === '1.6.9');
check('Versione lock 1.6.9', lock.version === '1.6.9' && lock.packages?.['']?.version === '1.6.9');
check('Versione Expo 1.6.9', app.expo.version === '1.6.9');
check('Android versionCode 17', app.expo.android?.versionCode === 17);
...
check('Manifest PWA 1.6.9', manifest.description?.includes('1.6.9'));
...
check('Service Worker 1.6.9', sw.includes("marilab-mover-shell-v1.6.9"));
...
check('Login HD 1580 px', auth.includes('maxWidth: 1580') && auth.includes('Enterprise 1.6.9'));
...
check('Conferma elimina viaggi compatibile Web', shell.includes("window.confirm(`Eliminare il viaggio chiuso?"));
...
check('Eliminazione messaggio', shell.includes('Elimina messaggio') && migration.includes('delete_chat_message'));
check('Svuotamento chat Admin', shell.includes('Svuota chat') && migration.includes('admin_clear_chat_conversation'));
...
check('Badge sincronizzato', store.includes('syncAppBadgeCount'));
...
console.log(`Collaudo statico E1.6.9: ${passed.length} controlli superati.`);
